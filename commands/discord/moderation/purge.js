...
export default {
  async f ({ embed, content, m }) {
    ...
    if ((m.channel.lastMessage || await m.channel.messages.fetch({ limit: 1 }, true)) && m.channel.lastMessage.createdTimestamp + 1.2096e9 < Date.now() || !m.channel.lastMessage.deletable) { return await (await m.channel.send(embed.a('Purge: Error', m.author.avatarURL()).d('```diff\n- Error: No deletable messages in this channel.```'))).delete({ timeout: 10000 }) }

    // Determines filter
    const filter = Object.keys(filters).find(f => (new RegExp(f)).test(content))

    // If the message filter didn't get filtered out(lol), return an error
    if (!filter) { return await (await m.channel.send(embed.a('Purge: Error', m.author.avatarURL()).d(`\`\`\`diff\n- Error: Couldn't find the filter for the messages you were looking for.\nTry doing '${this.prefix}help purge' to know what kind of filters exist.\`\`\``))).delete({ timeout: 10000 }) }

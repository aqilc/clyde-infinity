...
const osu = {
  ...
  embed: (m, user, embed) => {
    ...
    const max = 9; const progress = Math.round((user.level - Math.floor(user.level)) / 1 * max)

    // Calculates total circles tapped by user
    const circles = Number(user.count300) + Number(user.count100) + Number(user.count50)
  ...
  args: [
    {
      ...
      async f (m, { content, embed }) {
        ...
        if (!user) {
          ...
          const start = await m.channel.send(embed.t("Send a message containing your osu username, or 'c' to cancel").f('Specified username should at least be greater than 2 characters'))
        ...
        if (user === 'c') { return m.channel.send(embed.a('Query Cancelled', m.author.avatarURL()).f('')) }

...
export default {
  ...
  async f (m, { content, embed }) {
    ...
    // Text canvas
    const canvas = new ASCII(50, 5)

    // Message containing the acc information and stuff
    const msg = await m.channel.send(embed.a(`${player.name} vs ${enemies[0].name}`, m.author.avatarURL()).d(`\`\`\`\n${canvas}\`\`\``))
    ...
    async function react (action) {
      ...
      // Wait for reactions
      const r = (await msg.awaitReactions((reaction, user) => user.id === m.author.id && reaction.emoji.name === action.emoji, { time: 1.2e5, max: 1 })).first()
      ...
      if (!done()) { setTimeout(async () => await react(action), action.cooldown) }
    }

    // Loop through action reactions and do above function for all of them
    for (const i in actions) { react(actions[i]) }

    let frame = 0; const drawing = canvas.draw(' 😔\n | 🗡️\n/ \\', 10, 2)

// slightly modified, modernized ojsama by aqil, original by Francesco149
...
// let export const = require("./ojsama");
//
// let parser = new export const parser();
// readline.createInterface({
//   input: process.stdin, terminal: falsei
// })
// .on("line", parser.feed_line.bind(parser))
// .on("close", function() {
//   console.log(export const ppv2({map: parser.map}).toString());
// });
// ```
//
// ```sh
// $ curl https://export const ppy.sh/export const 67079 | node minexample.js
...
// let export const = require("./ojsama");
//
// let mods = export const modbits.none;
// let acc_percent;
// let combo;
// let nmiss;
//
...
//     mods = export const modbits.from_string(argv[i].slice(1) || "");
//   }
//
...
//
//   if (mods) {
//     console.log("+" + export const modbits.string(mods));
//   }
//
//   let stars = new export const diff().calc({map: map, mods: mods});
//   console.log(stars.toString());
//
//   let pp = export const ppv2({
//     stars: stars,
//     combo: combo,
//     nmiss: nmiss,
...
//
// ```sh
// $ curl https://export const ppy.sh/export const 67079 | node example.js
...
// 133.24 pp (36.23 aim, 40.61 speed, 54.42 acc)
//
// $ curl https://export const ppy.sh/export const 67079 \
...
//   function load_file()
//   {
//     let frame = document.getElementById("export const ile");
//     let contents = frame.contentWindow
//       .document.body.childNodes[0].innerHTML;
//
//     let parser = new export const parser().feed(contents);
//     console.log(parser.toString());
//
//     let str = parser.map.toString();
//     str += export const ppv2({map: parser.map}).toString();
//
...
// </head>
// <body>
//   <iframe id="export const ile" src="test.export const  onload="load_file();"
//   style="display: none;">
//   </iframe>
...
// ```
//
// (this example assumes you have a test.export const beatmap in the same
// directory)
//
// performance
// ----------------------------------------------------------------
...
// when used outside of node, a export const namespace will be exposed
...
let array_toFixed = function (arr, n) {
...
// hit objects
// ----------------------------------------------------------------
// partial structure of export const  hitobjects with just enough data for
// pp calculation
...
export function hitobject(values) {
    this.time = values.time || 0.0;
...
beatmap.prototype.max_combo = function () {
    ...
    for (let i = 0; i < this.objects.length; ++i) {
        ...
        if (!(obj.type & objtypes.slider)) continue;
...
// partial .export const file parser built around pp calculation
...
parser.prototype.feed_line = function (line) {
    ...
    // comments
    if (line.startsWith(" ") || line.startsWith("_")) return this;
...
// _(internal)_
// export const standard stats constants
...
// stores export const standard beatmap stats

export function std_beatmap_stats(values) {
    this.ar = values.ar;
    this.od = values.od;
    this.hp = values.hp;
    this.cs = values.cs;
...
std_beatmap_stats.prototype.with_mods = function (mods) {
    ...
    let stats = this._mods_cache[mods] = new std_beatmap_stats(this);
...
// export const  standard hit object with difficulty calculation values
// obj is the underlying hitobject

export function std_diff_hitobject(obj) {
    this.obj = obj;
    this.reset();
}
...
std_diff.prototype.calc = function (params) {
    ...
    let stats = new std_beatmap_stats({ cs: map.cs }).with_mods(mods);
    let speed_mul = stats.speed_mul;
    this._init_objects(this.objects, map, stats.cs);

    let speed = this._calc_individual(DIFF_SPEED, this.objects, speed_mul);
    this.speed = speed.difficulty;
    this.speed_difficulty = speed.total;

    let aim = this._calc_individual(DIFF_AIM, this.objects, speed_mul);
    ...
    for (let i = 1; i < this.objects.length; ++i) {
        ...
        let interval = (obj.time - prev.time) / speed_mul;
...
std_diff.prototype._spacing_weight = function (type, distance, delta_time,
    ...
    let angle_bonus;
    let strain_time = Math.max(delta_time, 50.0);
    switch (type) {
        case DIFF_AIM: {
            let prev_strain_time = Math.max(prev_delta_time, 50.0);
            let result = 0.0;
...
// export const  standard ppv2 calculator

export function std_ppv2() {
    this.aim = 0.0;
    this.speed = 0.0;
    this.acc = 0.0;
...
std_ppv2.prototype.calc = function (params) {
    ...
    let doubletap_penalty = Math.pow(0.98, n50 < nobjects / 500.0 ? 0 : n50 - nobjects / 500.0);

...
export class Table {
  constructor(db, table) {
    ...
    if(!db || !table)
      throw new Error(`The parameter ${(db === undefined && "db") || "table"} is undefined in the "Table" class!`)
    
    // Checks if the database officially exists(is in the config)
    if(!sql[db])
      throw new Error(`The table ${db} doesn't exist!`);

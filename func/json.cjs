...
export default class JSONDB {
  constructor (file, indent) {
    ...
    if (indent) { this._indent = indent }

    // Returns proxy that does things
    return new Proxy(this, {
      set (obj, prop, val) {
        if (prop.startsWith('_')) { throw new Error("Properties starting with \"_\" aren't allowed!") }
        obj._edit(prop, val)
      },
  ...
  get _update () {
    ...
    function update () {
      ...
      for (const i in self._file) {
        if (!(i in self)) { self._configVal(i) }
      }
    }
    update.file = () => {
